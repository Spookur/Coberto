...
namespace GuildCars.IntegrationTests.ReportsRepositoryTests
{
    ...
    public class ReportsRepositoryTestsMock
    {
        ...
        public void CanGetInventoryReport()
        {
            ...
            Assert.AreEqual(11, inventoryReportsList.Count);

            Assert.IsTrue(inventoryReportsList[0].IsNew);
            Assert.AreEqual("Nissan", inventoryReportsList[0].Make);
            Assert.AreEqual("Rogue", inventoryReportsList[0].Model);
            Assert.AreEqual(3, inventoryReportsList[0].UnitsInStock);
            Assert.AreEqual(155445m, inventoryReportsList[0].StockValue);
            Assert.AreEqual(new DateTime(2021,1,1), inventoryReportsList[0].Year);
        }
