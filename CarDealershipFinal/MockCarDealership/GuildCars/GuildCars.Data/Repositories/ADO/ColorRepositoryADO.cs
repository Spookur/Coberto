...
namespace GuildCars.Data.Repositories.ADO
{
    public class ColorRepositoryADO : IColorRepository
    {
        public IEnumerable<Color> GetAll()
        {
            ...
            using (var dbConnection = new SqlConnection(Settings.GetConnectionString())) // use and connect to the database via the correct connection strings
            {
                try
                {
                    ...
                    using (SqlDataReader dr = cmd.ExecuteReader())
                    {
                        while (dr.Read())
                        {
                            ...
                            Color.ColorId = (int)dr["ColorId"]; // read database for the color id
                            Color.ColorName = dr["ColorName"].ToString(); // read database for the color name
